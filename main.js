...
function listCustomers(personArray) {
  ...
  for (let customer of customers) {
    ...
    let customerText = document.createTextNode(`${customer.name.title}. ${customer.name.first} ${customer.name.last} located on ${customer.location.street}  ${customer.location.city}, ${customer.location.state}`)
...
//Name
customerList.append(newCustomer);
newCustomer.append(customerText);
//newCustomer.classList.add("display");

//Email
email.classList.add("email");
newEmail.append(emailText);
//newEmail.classList.add("display");